...
function MascotasPage() {
  ...
  const filteredDogs = useMemo(() => {
    ...
    return dogs.filter((dog) => dog.image).filter((dog) => (
      ...
      && (!normalizedSearch || `${dog.name} ${dog.breed}`.toLowerCase().includes(normalizedSearch))
    ));
    ...
    <div className="site-shell">
      <header className="site-header">
        ...
        <nav className="site-nav" aria-label="Navegación principal">
          <Link to="/">Inicio</Link>
          <Link to="/mascotas" aria-current="page">Mascotas</Link>
          <Link to="/#como-adoptar">Cómo adoptar</Link>
          <Link to="/#nosotros">Sobre nosotros</Link>
        </nav>

        {user ? (
          <button className="session-button" type="button" onClick={handleLogout}>Cerrar sesión</button>
        ) : (
          <button className="login-button" type="button" onClick={() => navigate("/login")}>Iniciar sesión</button>
        )}
      </header>
      ...
      <main className="catalog-main">
        <div className="catalog-heading">
          ...
          <p>Usa los filtros para encontrar al perrito ideal para tu familia.</p>
        </div>
        ...
        <div className="catalog-layout">
          ...
          <section className="catalog-results" aria-labelledby="catalog-results-title">
            <div className="catalog-toolbar">
              <div>
                <h2 id="catalog-results-title">Perritos disponibles</h2>
                <p>{filteredDogs.length} {filteredDogs.length === 1 ? "resultado" : "resultados"}</p>
              </div>
